...
const index = () => {
  ...
  const addTodo = async () => {
    try {
      ...
      axios
        .post(`${Constants.url}/todos/665f59fc1c90989709cb44f5`, todoData)
        .then((response) => {
      ...
      setTodo("");
      Alert.alert("Todo updated","New item added successfully")
    } catch (error) {
      console.log("error", error);
    }
  };
  ...
  const getUserTodos = async () => {
    try {
      const response = await axios.get(
        `${Constants.url}/users/665f59fc1c90989709cb44f5/todos`
      );
  ...
  const markTodoAsCompleted = async (todoId) => {
    try {
      ...
      const response = await axios.patch(
        `${Constants.url}/todos/${todoId}/complete`
      );
